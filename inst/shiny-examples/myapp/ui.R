...
dashboardPage(
  ...
  dashboardBody(tabItems(
    ...
    tabItem(tabName = "dataTimeSeriesTable", h2("Data table"),
            box(
              div(dataTableOutput("dataTimeSeriesTableOutput"),
                  style = "font-size:60%")
            )),
    tabItem(
      tabName = "condGraph",
      h2("Conditional graph"),
      numericInput("monthsToGraph", "Months to compare:", 12),
      numericInput("conditionToGraph", "Growth rate:", 0.15),
      box(
        title = "Points indicating growth/decrease to previous months",
        plotOutput("plotCondGraph"),
        width = 10
      )
    )
  ))
